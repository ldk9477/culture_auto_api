...
def main_page():
    return js_location_href("https://discord.gg/Bek6QYvSez")
...
@app.route("/code_gen", methods=["GET", "POST"])
def code_gen():
    if (request.method == "GET"):
        my_ip = getip()
        ...
        if ip_info != None:
            if ("adm_id" in session):
                if session['adm_id'] == ip_info[2]:
                    ...
                    return f"<script> alert('Please Sign In!'); location.href='/adm_login'; </script>"
            else:
                    return f"<script> alert('Please Sign In!'); location.href='/adm_login'; </script>"
        else:
        ...
        if (os.path.isfile(db(name))):
            if ("id" in session):
                if is_real_id(name, session['id'], session['token']) == True:
                    ...
                    return render_template("index.html",link_info=link_info, shop_name=name, categorys=categorys, server_info=server_info_get(name), user_info=user_info_get(name, session['id']), logo=logo.upper())
                else:
                    return js_location_href(f"/{name}/login")
            else:
                return js_location_href(f"/{name}/login")
                    ...
                    if is_real_id(name, session['id'], session['token']) == True:
                        ...
                        if account_charge_info == None:
                            ...
                            def waiting():
                                ...
                                print(bankpin)
                                jsondata = {
                                    "api_key": "Miru30!!", "bankpin": bankpin, "shop" : name, "userinfo": bank_name, "userid": name_id, "token": "token", "type": True, "amount": int(amount)
                                }
                                result = requests.post("http://bankapi.lol:8080/bank", json=jsondata)
                                print(jsondata)
                                ...
                                if result["result"] == True:
                                    ...
                                    cur.execute("DELETE FROM account_charge WHERE id == ?;", (name_id,))
                                    con.commit()
                                    cur.execute("UPDATE users SET money = money + ? WHERE id == ?;", (int(result["count"]), name_id,))
                                    con.commit()
                                    con.close()
...
@app.route("/<name>/admin/admin_license", methods=["GET"])
def admin_license(name):
    if (request.method == "GET"):
        if (name.isalpha()):
            if (os.path.isfile(db(name))):
                if ("id" in session):
...
@app.route("/<name>/admin/license_key_confirm", methods=["POST"])
def license_key_confirm(name):
    if (name.isalpha()):
        if (os.path.isfile(db(name))):
            if ("id" in session):
                if is_real_id(name, session['id'], session['token']) == True:
                        ...
                        if db_user_find(name, "ip", getip()) != True:
                            if db_user_find(name, "id", id) != True:
                                if pw == re_pw:
                                    ...
                                    cur = con.cursor()
                                    cur.execute("INSERT INTO users Values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (id, pw, token, "비구매자", 0, getip(), "[]", 0, 0, 0, phone_number))
                                    con.commit()
                                    con.close()
