...
def charge(id, pw, code, cookie):
    code = code.split("-")

    not_code = {"success": False, "amount": 0, "result": "잘못된 핀코드 형식", "time": 0}
    ...
    sess.cookies.set("KeepLoginConfig", cookie, domain="m.cultureland.co.kr") #쿠키 KeepLoginConfig
    ...
    if k == "false":
        return {"success": False, "amount": 0, "result": "잘못된 계정 정보"}
    ...
    if result == "컬쳐캐쉬로 충전 불가능":
        return {"success": False, "amount": 0, "result": result}
    else:
        amount = int(soup.select("dd")[0].text.replace("원", "").replace(",", ""))
        return {"success": bool(amount), "amount": amount, "result": result}
