    ...
    if not api == None:
        ...
        if not(is_expired(cmdchs[2])):
            ...
            if not ip == None:
                ...
                result = charge(id, pw, pin, cookie)
                return result
            else:
                return {"success": False, "amount": 0, "result": "아이피가 올바르지 않습니다."}
        else:
            return {"success": False, "amount": 0, "result": "만료된 라이센스입니다."}
    else:
        return {"success": False, "amount": 0, "result": "API키가 올바르지 않습니다."}


app.run("0.0.0.0", port=123)
